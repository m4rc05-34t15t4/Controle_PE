        ...
        $sql = "
            UPDATE public.cartas SET status = $status_destino, \"$funcao\" = '$usuario', \"inicio$funcao\" = '$data_inicio' WHERE id =  
                CASE
                    ...
                    WHEN (SELECT id FROM public.cartas WHERE cast(status as float) = $status_origem AND \"$funcao\" isnull AND (\"inicio$funcao\" isnull OR length(\"inicio$funcao\") < 1) AND niveis = '$primeira_dificuldade' ORDER BY bloco, mi LIMIT 1) > 0 THEN (SELECT id FROM public.cartas WHERE cast(status as float) = $status_origem AND \"$funcao\" isnull AND (\"inicio$funcao\" isnull OR length(\"inicio$funcao\") < 1) AND niveis = '$primeira_dificuldade' ORDER BY prioridade DESC, bloco, mi LIMIT 1) --<br>
                    WHEN (SELECT id FROM public.cartas WHERE cast(status as float) = $status_origem AND \"$funcao\" isnull AND (\"inicio$funcao\" isnull OR length(\"inicio$funcao\") < 1) AND niveis = '$segunda_dificuldade' ORDER BY bloco, mi LIMIT 1) > 0 THEN (SELECT id FROM public.cartas WHERE cast(status as float) = $status_origem AND \"$funcao\" isnull AND (\"inicio$funcao\" isnull OR length(\"inicio$funcao\") < 1) AND niveis = '$segunda_dificuldade' ORDER BY prioridade DESC, bloco, mi LIMIT 1) --<br>
