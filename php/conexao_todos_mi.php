...
$lista_edicao = Busca_dados("SELECT mi, status, niveis, \"RevHid\", \"AqHid\", \"AdVet\", \"CQ1\", editor, revisor1, corretor1, revisor2, corretor2, \"CQ2\", servidor FROM public.cartas ORDER BY mi"); 

if(count($lista_edicao) > 0){
    
    $lista_usuarios = Busca_dados("SELECT codigo, nome FROM public.usuarios ORDER BY nome");
    
    $lista_fases_edicao = Busca_dados("SELECT * FROM public.\"fases\" ORDER BY codigo"); 
...
function Busca_dados_edicao(){
    ...
    $sql = "SELECT mi, status, niveis, \"RevHid\", \"AqHid\", \"AdVet\", CQ1, editor, revisor1, corretor1, revisor2, corretor2, CQ2, servidor FROM public.cartas ORDER BY mi";
