<?php
    ...
    elseif( ($_GET["dificuldade"] != "Fácil") AND ($_GET["dificuldade"] != "Média") AND ($_GET["dificuldade"] != "Difícil") ){
        ...
        $sql = "
            ...
            CREATE OR REPLACE FUNCTION QTD_MI_LOCAL(_dificuldade text) RETURNS bigint AS $$
                BEGIN
                    RETURN 
                        ...
                        (SELECT id FROM public.cartas WHERE status = '2' AND niveis = _dificuldade AND (editor isnull OR  length(editor) < 1) AND (inicioedicao isnull OR length(inicioedicao) < 1) ORDER BY bloco, mi LIMIT 1);
                END;
            $$ LANGUAGE plpgsql;
            ...
            CREATE OR REPLACE FUNCTION QTD_MI(_dificuldade text) RETURNS bigint AS $$
                BEGIN
                    RETURN 
                        (SELECT id FROM
                            --RETORNA MI SOLICITADO<br>
                            (SELECT * FROM public.cartas WHERE status = '2' AND niveis = _dificuldade AND (editor isnull OR  length(editor) < 1) AND (inicioedicao isnull OR length(inicioedicao) < 1) ORDER BY bloco, mi)
            ...
            UPDATE public.cartas SET status = '3', editor = '$editor', inicioedicao = '$data_inicio_edicao' WHERE id =  
                CASE
                    WHEN
                        ...
                        ((SELECT id FROM public.cartas WHERE status = '2' AND editor = '$editor' AND (inicioedicao isnull OR length(inicioedicao) < 1) ORDER BY bloco LIMIT 1) > 0)
                    THEN
                        --retornar id carta reservada<br>
                        (SELECT id FROM public.cartas WHERE status = '2' AND editor = '$editor' AND (inicioedicao isnull OR length(inicioedicao) < 1) ORDER BY bloco LIMIT 1)
                    WHEN
                        --retorna true se o usuario for local<br>
                        ((SELECT srv_local FROM public.usuarios WHERE codigo = CAST('$editor' as integer)) = true) AND
                    ...
                    WHEN 
                        ...
                        (SELECT id FROM
                            ...
                            (SELECT * FROM public.cartas WHERE status = '2' AND editor = '$editor' AND (inicioedicao isnull OR length(inicioedicao) < 1) ORDER BY bloco)
