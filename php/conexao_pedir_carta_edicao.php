<?php
    ...
    elseif( ($_GET["dificuldade"] != "Fácil") AND ($_GET["dificuldade"] != "Média") AND ($_GET["dificuldade"] != "Difícil") ){
        ...
        if(!isset($_GET["dificuldade"])) $primeira_dificuldade = "Difícil";
            ...
            UPDATE public.cartas SET status = $status_destino, $funcao = '$usuario', inicioedicao = '$data_inicio' WHERE id =  
                CASE
                    WHEN 
                        ...
                        (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao = '$usuario' AND (inicioedicao isnull OR length(inicioedicao) < 1) ORDER BY prioridade DESC, bloco, mi LIMIT 1) > 0
                    THEN
                        (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao = '$usuario' AND (inicioedicao isnull OR length(inicioedicao) < 1) ORDER BY prioridade DESC, bloco, mi LIMIT 1)
                    
                    --retorna se há cartas disponiveis<br>                   
                    WHEN (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao isnull AND (inicioedicao isnull OR length(inicioedicao) < 1) AND niveis = '$primeira_dificuldade' ORDER BY prioridade DESC, bloco, mi LIMIT 1) > 0 THEN (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao isnull AND (inicioedicao isnull OR length(inicioedicao) < 1) AND niveis = '$primeira_dificuldade' ORDER BY prioridade DESC, bloco, mi LIMIT 1)
                    WHEN (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao isnull AND (inicioedicao isnull OR length(inicioedicao) < 1) AND niveis = '$segunda_dificuldade' ORDER BY prioridade DESC, bloco, mi LIMIT 1) > 0 THEN (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao isnull AND (inicioedicao isnull OR length(inicioedicao) < 1) AND niveis = '$segunda_dificuldade' ORDER BY prioridade DESC, bloco, mi LIMIT 1)
                    WHEN (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao isnull AND (inicioedicao isnull OR length(inicioedicao) < 1) AND niveis = '$terceira_dificuldade' ORDER BY prioridade DESC, bloco, mi LIMIT 1) > 0 THEN (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao isnull AND (inicioedicao isnull OR length(inicioedicao) < 1) AND niveis = '$terceira_dificuldade' ORDER BY prioridade DESC, bloco, mi LIMIT 1)
