<?php
    ...
    if( isset($_SESSION['login']) AND isset($_SESSION['senha']) AND isset($_SESSION['codigo']) OR isset($_SESSION['usuario']) AND isset($_SESSION['funcao']) ){
        ...
        if($_SESSION['funcao'] > 64){
            ...
            $sql = "
                SELECT mi, inicio2rev AS data, bloco FROM public.cartas WHERE revisor2 = '$usu_codigo' AND inicio2rev IS NOT null
                UNION 
                SELECT mi, termino2rev AS data, bloco FROM public.cartas WHERE revisor2 = '$usu_codigo' AND termino2rev IS NOT null
                UNION
                SELECT mi, inicio1rev AS data, bloco FROM public.cartas WHERE revisor1 = '$usu_codigo' AND inicio1rev IS NOT null
                UNION
                SELECT mi, termino1rev AS data, bloco FROM public.cartas WHERE revisor1 = '$usu_codigo' AND termino1rev IS NOT null
                UNION
                SELECT mi, inicioedicao AS data, bloco FROM public.cartas WHERE editor = '$usu_codigo' AND inicioedicao IS NOT null
                UNION
                SELECT mi, terminoedicao AS data, bloco FROM public.cartas WHERE editor = '$usu_codigo'  AND terminoedicao IS NOT null
