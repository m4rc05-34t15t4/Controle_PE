            ...
            UPDATE public.cartas SET status = $status_destino, $funcao = $usuario, $inicio = '$data_inicio' WHERE id =  
                CASE
                    WHEN 
                        ...
                        (SELECT id FROM public.cartas WHERE status = $status_origem AND ($funcao = '$usuario' OR editor = '$usuario') AND ($inicio isnull OR length($inicio) < 1) ORDER BY prioridade DESC, bloco, mi LIMIT 1) > 0
                    THEN
                        (SELECT id FROM public.cartas WHERE status = $status_origem AND ($funcao = '$usuario' OR editor = '$usuario') AND ($inicio isnull OR length($inicio) < 1) ORDER BY prioridade DESC, bloco, mi LIMIT 1)
                                       
                    WHEN
                        --retorna se há cartas disponiveis<br> 
                        (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao isnull AND ($inicio isnull OR length($inicio) < 1) ORDER BY prioridade DESC, bloco, mi LIMIT 1) > 0 
                    THEN 
                        (SELECT id FROM public.cartas WHERE status = $status_origem AND $funcao isnull AND ($inicio isnull OR length($inicio) < 1) ORDER BY prioridade DESC, bloco, mi LIMIT 1)
